...
class UserValidation
{
    ...
    public function handle($request, Closure $next) {
        $user = DB::select("SELECT * FROM users inner join entity_user on entity_user.user_id = users.id and entity_user.deleted_at is null
                            where (users.email = '{$request->input('email')}' or users.user_id = '{$request->input('user_id')}' 
                            or users.rfid_number ='{$request->input('rfid_number')}' 
                            or users.phone ='{$request->input('phone')}') 
        ...
        if ($user) {
            ...
            foreach ($request->all() as $key => $input) {
                ...
                if ($key == 'rfid_number' && $input == $user[0]->rfid_number) {
